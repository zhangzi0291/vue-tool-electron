...
export default defineComponent({
  ...
  data() {
    return {
      ...
      bgImg: "",
      sliderImg: "",
      startSlidingTime: {} as Date,
      entSlidingTime: {} as Date,
      trackArr:[] as any
    }
  },
  methods: {
    ...
    refreshCaptcha() {
      this.$api.SysLoginApi.gen().then(res => {
        this.id = res.data.id;
    ...
    valid() {
      const sliderImgDiv = this.$refs.sliderImgDiv as any
      let data = {
        bgImageWidth: this.bgImgWidth,
        bgImageHeight: sliderImgDiv.offsetHeight,
        sliderImageWidth: sliderImgDiv.offsetWidth,
        sliderImageHeight: sliderImgDiv.offsetHeight,
        startSlidingTime: this.startSlidingTime,
        entSlidingTime: this.entSlidingTime,
        trackList: this.trackArr
      };
      this.$api.SysLoginApi.check(data).then(res => {
